...
import { Flame, Bell, Search, Menu, Sparkles, Check, CheckCircle2, Info, LogOut, User, Trophy } from "lucide-react";
          ...
          <div className="relative" ref={profileRef}>
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-10 h-10 rounded-xl bg-primary-blue flex items-center justify-center text-white font-black shadow-lg shadow-primary-blue/20 overflow-hidden border-2 border-white shrink-0 hover:shadow-lg hover:shadow-primary-blue/40 transition-all cursor-pointer"
              >
                {session?.user?.image ? (
                  <img src={session.user.image} alt="Profil" className="w-full h-full object-cover" />
                ) : (
                  <span>{(session?.user?.name?.[0] || "P").toUpperCase()}</span>
                )}
              </button>
            ...
            <AnimatePresence>
              {isProfileOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10, scale: 0.95 }}
                  transition={{ duration: 0.2 }}
                  className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-light-blue overflow-hidden flex flex-col z-50 top-full"
                >
                  {/* User Info */}
                  <div className="p-4 bg-gradient-to-r from-primary-blue/5 to-transparent border-b border-light-blue">
                    <p className="text-sm font-black text-dark-blue">{session?.user?.name || "Pengguna"}</p>
                    <p className="text-xs text-slate-500 font-medium mt-1 overflow-hidden">{session?.user?.email}</p>
                  </div>
                  ...
                  <div className="divide-y divide-light-blue">
                    <Link
                      ...
                      className="flex items-center gap-3 p-4 text-dark-blue hover:bg-light-blue/50 transition-colors font-bold"
                    >
                      <User size={18} />
                      <span>Profil</span>
                    </Link>
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 p-4 text-red-500 hover:bg-red-50/50 transition-colors font-bold w-full text-left"
                  >
                    <LogOut size={18} />
                    <span>Keluar</span>
                  </button>
            ...
            {/* FIX 2: Removed the stray duplicate avatar/closing fragment that appeared here */}
          </div>
        </div>
      </div>
    </header>
