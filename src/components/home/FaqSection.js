...
function FaqItem({ question, answer, isOpen, onClick }) {
  return (
    <div className="group rounded-[1.5rem] border border-slate-200 bg-white p-2 shadow-sm transition-all hover:border-skillio-300">
      <button className="flex w-full items-center justify-between px-4 py-4 text-left sm:px-6" onClick={onClick}>
        <span className="font-display text-base font-bold text-slate-900 sm:text-lg">{question}</span>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <p className="px-4 pb-6 pt-2 text-sm leading-relaxed text-slate-600 sm:px-6 sm:text-base">{answer}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    <section id="faq" className="relative border-y border-slate-100 bg-slate-50/50 px-5 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-12 lg:grid-cols-[1fr_1.5fr] lg:gap-16">
          <Reveal direction="right" className="flex flex-col items-start space-y-6">
          ...
          <Reveal direction="left" delay={0.1} className="flex flex-col space-y-4">
            {faqs.map((faq, index) => (
              <FaqItem
                key={index}
